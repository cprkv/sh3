...
template <class T>
inline auto print_helper(std::ostream& os, const T& t, int, int)
    -> decltype(os << t)& {
    return os << t;
}
...
inline auto print_helper(std::ostream& os, const std::pair<First, Second>& t, int,
...
inline auto print_sequence_helper(std::ostream &os, const Sequence& seq)
...
template <class Sequence>
inline auto print_helper(std::ostream& os, const Sequence& seq, int, ...)
...
inline std::ostream& print_helper(std::ostream& os, const Elem (&arr)[N], int, int) {
...
inline std::ostream& print_helper(std::ostream& os, const char (&arr)[N], int, int) {
...
template <class T>
inline std::ostream& print_helper(std::ostream& os, const T&, ...) {
...
template <typename First, typename Second>
inline bool safe_equals_helper(const First& first, const Second& second) {
  return first == second;
}

template <typename First, typename Second>
inline bool safe_not_equals_helper(const First& first, const Second& second) {
  return first != second;
}

// Allow size_t to correctly be compared to int.
inline bool safe_equals_helper(std::size_t first, int second) {
  return second >= 0 && static_cast<long long>(first) == second;
}

inline bool safe_equals_helper(int first, std::size_t second) {
  return first >= 0 && first == static_cast<long long>(second);
}

inline bool safe_not_equals_helper(std::size_t first, int second) {
  return second < 0 || static_cast<long long>(first) != second;
}

inline bool safe_not_equals_helper(int first, std::size_t second) {
  return first < 0 || first != static_cast<long long>(second);
}
...
inline void TestCase::run(bool quiet_mode) {
    try {
        if (not quiet_mode) {
...
inline void TestCase::print(bool quiet_mode) {
    if (quiet_mode) {
        std::cout << name << ": ";
    }
    else {
...
inline int TestSuite::run_tests(int argc, char** argv) {
...
inline std::vector<std::string> TestSuite::get_test_names_to_run(int argc,
                                                          char** argv) {
    std::vector<std::string> test_names_to_run;
#if UNIT_TEST_ENABLE_REGEXP
...
inline std::ostream& operator<<(std::ostream& os, const TestFailure& test_failure) {
...
inline void assert_true(bool value, int line_number, const char* assertion_text) {
    if (value) {
        return;
    }
    std::ostringstream reason;
...
inline void assert_false(bool value, int line_number, const char* assertion_text) {
    if (not value) {
        return;
    }
    std::ostringstream reason;
...
inline void assert_almost_equal(double first, double second, double precision,
